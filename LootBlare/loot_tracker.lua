...
local function initDB()
    ...
    if LootTrackerDB.showText == nil then
        LootTrackerDB.showText = true
    end
...
local function buildItemLink(item)
    ...
    item.realLink =
        "|c"..color.."|Hitem:"..item.itemID.."|h["..name.."]|h|r"
end
...
function updateLootTrackerLayout()
    ...
    lootTrackerFrame:SetWidth(horizontal and total or lootTrackerFrame.rows[1]:GetWidth())
    lootTrackerFrame:SetHeight(horizontal and lootTrackerFrame.rows[1]:GetHeight() or total)
...
function createLootTrackerFrame()
    ...
    f:SetScript("OnMouseWheel", function()
        ...
        currentScale = math.max(
            SCALE_MIN,
            math.min(SCALE_MAX, currentScale + (delta > 0 and SCALE_STEP or -SCALE_STEP))
        )
